...
//                 std::ostringstream msg_text_stream;
//                 msg_text_stream << "Some message content";

//                 std::string msg_text = msg_text_stream.str();
...
void check_conf(struct config_t &conf)
{
    if (conf.chat_id.empty())
    {
        std::cerr << "Chat id is still empty!\n";
        exit(1);
    }
    if (conf.token.empty())
    {
        std::cerr << "Token is still empty!\n";
        exit(1);
    }
    if (conf.repos.empty())
    {
        std::cerr << "Repo list is still empty!\n";
        exit(1);
    }
}
...
bool check_commit(std::string latest_commit_sha, std::string &repo)
{
    std::string _path = std::string(DATA_FOLDER_PATH).append(repo, 0, repo.find_first_of('/'));
    std::string filepath = std::string(DATA_FOLDER_PATH).append(repo);
    if (!is_exists(_path))
    ...
    if (infile.is_open())
    {
        std::string sha;
        infile >> sha;
        if (sha == latest_commit_sha)
            return false;
        else
        {
            infile.close();
            std::ofstream outfile(filepath);
            outfile << latest_commit_sha;
            outfile.close();
            return true;
        }
    }
...
void response_code_check(long &response_code, std::string repo, nlohmann::json j)
{
    if (response_code == 403)
    {
        std::cerr << "API Limit Exceeded! To increase limit use a GitHub personal token\n";
    }
    else if (response_code == 404)
    {
        std::cerr << "Commits not found in " << repo << "\n";
    }
    else
    {
        if (j["message"] == nullptr)
...
int main()
{
    ...
    while (true)
    {
        ...
        {
            std::string response_string;
            std::string url = std::string("https://api.github.com/repos/").append(repo).append("/commits?per_page=1");
            curl.get_page(url.c_str(), response_string, conf.personal_token);
            ...
            if (curl.response_code != 200)
            {
                response_code_check(curl.response_code, repo, j);
                std::ostringstream msg_text_stream;
                msg_text_stream << "Some message content";
            ...
            std::string latest_commit_sha = j[0]["sha"].get<std::string>();
            std::string commit_message = j[0]["commit"]["message"];
            std::string commit_url = j[0]["html_url"];
            std::string author_name = j[0]["commit"]["author"]["name"];
            std::string repo_name = repo.substr(repo.find_first_of('/') + 1);

            if (check_commit(latest_commit_sha, repo))
            {
                std::string github = "https://github.com/";
                std::string repo_link = std::string("https://github.com/").append(repo);
                std::string author_link = std::string("https://github.com/").append(author_name);
                std::string msg_text;
                std::stringstream ss;
                ss << "<strong>New commit in <a href='" << repo_link << "'>" << repo_name << "</a></strong>\n"
                   << "<strong>Author: </strong><a href='" << author_link << "'>" << author_name << "</a>\n"
                   << "<strong>Commit Message: </strong><code>" << commit_message << "</code>\n"
                   << "<strong>Commit URL: </strong><a href='" << commit_url << "'>" << commit_url << "</a>\n";
                msg_text = ss.str();
                std::cout << "Sending message\n";
                telegram_message_send(conf, msg_text);
            }
